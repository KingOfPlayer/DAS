...
if (isset($_GET['action']) && $_GET['action'] == "search") {
	...
	// Sorguyu hazırla
	$sql = "SELECT 
		doctors.*,available_appointments.date AS 'nearest_date', citys.name AS 'city_name', doctor_specialtys.name AS 'specialty_name', doctor_degrees.name AS 'degree_name' 
		FROM
		doctors
		JOIN
		...
		JOIN
		available_appointments ON available_appointments.doctor_id = doctors.id
	
	
		WHERE citys.id = $sehir AND doctor_specialtys.id = $doctor_specialty  AND 'nearest_date' IS NOT NULL
		ORDER BY 'nearest_date' ASC
		LIMIT 1";
				...
				echo "<div class=\"w-50 d-flex justify-content-center pb-5\">
						<div class=\"card doctor-card\" style=\"width: 18rem;\">
							...
							<div class=\"card-body\">
								...
								<p class=\"card-text px-2\">En Yakın Randevu: " . $row['nearest_date'] . "</p>
    ...
    <body>
		...
		<script>
			$(document).ready(function () {
				...
				$.ajax({
					type: "POST",
					url: "getappointment.php?action=search",
					data: formData,
					dataType: "text",
					encode: true,
				}).done(function (data) {
